...
def main():
    ...
    if calc_choice == "1":
        ...
        # Ask about shave
        apply_shave = prompt_nonempty("Apply shave? (y/n): ").lower()
        if apply_shave == "y":
            while True:
                try:
                    shave_amount = float(prompt_nonempty("Enter shave amount in inches (default 1/16 = 0.0625): ")) or 0.0625
                    break
                except ValueError:
                    print("Please enter a numeric value for shave amount.")
            cut_length -= shave_amount
